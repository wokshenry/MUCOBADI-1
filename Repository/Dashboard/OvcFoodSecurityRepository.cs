...
{
    public class OvcFoodSecurityRepository : IOvcFoodSecurity
    {
        private readonly SPToCoreContext _context;

        public OvcFoodSecurityRepository(SPToCoreContext context)
        {
            _context = context;
        }

        public async Task<IEnumerable<spDashboard_OVC_FoodSecurityResult>> 
            GetByDistrict(DateTime? startDate, DateTime? endDate, 
                string districtId)
        {
            var data = await _context
              .spDashboard_OVC_FoodSecurityAsync
              (districtId, null, null);
            return data;
        }

        public Task<IEnumerable<spDashboard_OVC_FoodSecurityResult>> 
